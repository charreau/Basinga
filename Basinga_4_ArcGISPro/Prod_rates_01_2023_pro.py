...
fsp = 0.9886 #Martin et al., 2017 after Braucher et al. 2011
fmu_slowmuon=0.0027# Martin et al., 2017 after Braucher et al. 2011
fmu_fastmuon=0.0087# Martin et al., 2017 after Braucher et al. 2011
...
##Please note that the code was originally written for 10Be only, for simplicty we keep the same for the parameters with "_Be" but it does consider different nuclide
    if Input_scaling_scheme == "Stone":
        ...
        fmu_slowmuon=0.3/SLHLP_Be#see Blard et al. (2021) and Larsen et al. (2021)
        fmu_fastmuon=0
        fsp=1-fmu_slowmuon
...
def Rc_correctedStoneFactors(VDM, LAT_average, P_average, Time):
    ...
    Mid=5
    VectT = np.linspace(Time * 0.1, Time * 1.1, NbPoints)
...
def processVDM(input_file, VDM_in, input_correction_type):
    ...
    Teq = "$feature."+ NamFldConc + "/ $feature.Pneu_"+ input_correction_type
    
    arcpy.CalculateField_management(input_file, "Time", Teq, "arcade", "")
...
def processNumpy(DEM, MASK=None):
    ...
    if MASK is not None:  # Test if there is a mask
        ...
        if not np.array_equal(DEM_array.size, MASK_array.size):  # Test if both NoData-Data pixels are the same between DEM and MASK
            arcpy.AddError("<!> DEM and MASK don't have the exact same pixels with data <!>\n"
                            + "DEM:" + str(DEM_array.size) + " vs MASK:" + str(MASK_array.size)
...
def processNumpy_LSD_poly(DEM, MASK=None):
    ...
    for o in range(1,11):
        ...
        #arcpy.AddMessage(n_echantillon/10)
        
        if len(Total_S_st[indy])>n_echantillon/10:
            St_sp_extract=random.choices(SF_Be_neutron[indy],k=int(n_echantillon/10))
            St_sm_extract=random.choices(SF_Be_slowmuon[indy],k=int(n_echantillon/10))
            St_fm_extract=random.choices(SF_Be_fastmuon[indy],k=int(n_echantillon/10))
            DEM_extract=random.choices(DEM_bon[indy],k=int(n_echantillon/10))
            Lat_extract=random.choices(LAT_bon[indy],k=int(n_echantillon/10))
            Lon_extract=random.choices(LONG_bon[indy],k=int(n_echantillon/10))
